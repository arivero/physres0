...
\midrule
$-0.01$ & 2.09 & 1.55 & 0.74 & 0 & 0 \\
$-0.05$ & 2.39 & 1.55 & 0.65 & 0 & 0 \\
$-0.50$ & 3.72 & 1.56 & 0.42 & 1 & 0 \\
$-2.0$  & 5.63 & 1.59 & 0.28 & 2 & 0 \\
$-5.0$  & 7.79 & 1.62 & 0.21 & 3 & 0 \\
$-10.0$ & 10.13 & 1.65 & 0.16 & 4 & 0 \\
$-20.0$ & 13.30 & 1.66 & 0.12 & 6 & 0 \\
\bottomrule
\end{tabular}
\end{center}
\medskip
...
\item The Yukawa Bargmann integral grows monotonically with binding
  energy, crossing $I = 3$ at $E_0 \approx -0.5$ (enabling $L=1$) and
  $I = 5$ at $E_0 \approx -2$ (enabling $L=2$), generating a rich
  Regge tower.
...
The numerical computation (Table above) shows $I_{\text{fer}} \leq 1.66$
at all binding energies tested ($E_0$ from $-0.01$ to $-20$).  Since
$1.66 < 3 = 2\cdot 1+1$, we have $N_1 = 0$; and since $1.66 < 2L+1$
for all $L \geq 1$, no higher partial waves bind either.

A key subtlety strengthens this result.  The coupling $\lambda$ at each
binding energy is determined variationally (minimizing over the trial
parameter~$\alpha$).  Since the variational principle gives
$E_{\text{var}} \geq E_{\text{exact}}$, achieving a target binding
energy requires $\lambda_{\text{var}} \geq \lambda_{\text{exact}}$
(the trial wavefunction is less efficient than the true ground state,
so the coupling must compensate).  Therefore
$I_{\text{var}} \geq I_{\text{exact}}$: the Bargmann integrals in
the table are \emph{upper bounds} on the exact values.
...
$\int_0^\infty r^3 |V|\,\dd r < 1$.
Because $|V_{\text{fer}}| \sim \ee^{-2m_f r}/r^{7/2}$, the
$r^3$-weighted integral converges more strongly than the Bargmann
integral and offers a potentially sharper criterion for the absence
of $L \geq 1$ states.
...
For our short-range fermion potential ($|V| \sim \ee^{-2m_f r}/r^{7/2}$),
this integral converges strongly, providing an independent bound on
the total spectrum.
