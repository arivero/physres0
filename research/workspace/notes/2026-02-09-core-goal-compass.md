...
- Active support lane: semigroup→generator and Schur-complement determinant templates are now Lean-checked (`Claim1lean/SemigroupGenerator.lean`, `Claim1lean/SchurComplementDeterminant.lean`); next formalize semigroup/normalization theorems (Gaussian `t^{-d/2}` and Van Vleck-style prefactors) supporting the Newton-limit paradox framing and tighten reconstruction-facing interfaces.
