...
|---|---:|---|
| 1 | 9.4 | Scoped theorem package now includes non-factorized quadratic-mixing/quartic-tail large-\(N\) extensions, finite-\(g\) non-perturbative multi-mode quartic control (Euclidean + oscillatory), and an \(\eta\to0^+\) closure for that sector; remaining gap is full continuum/global interacting closure. |
