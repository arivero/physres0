...
"""Monte Carlo check for non-perturbative oscillatory multi-mode quartic closure.
...
def main() -> None:
    ...
    print("Claim 1 non-perturbative oscillatory multi-mode quartic check")
    print(f"c={c}, g={g}, kappa={g/c}, samples={samples}")
    print()

    vals: list[complex] = []
    for N in Ns:
        w = omega_N(N=N, c=c, g=g, samples=samples)
        vals.append(w)
        print(f"N={N:>2d}  omega_N={w.real:.10f} + {w.imag:.10f}i")
