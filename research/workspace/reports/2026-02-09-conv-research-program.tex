...
\title{From \texttt{conv\_patched.md} to a Research Program:\\
...
complex exponential weights \(\e^{\frac{i}{\hbar}S}\) and an oscillatory amplitude object whose
modulus-square yields the concentrating distribution (with geometric \(1/2\)-density
representation in kernel-composition sections).

We turned that slogan into a concrete, dimension-aware research program:
a three-level escalation (statics $\to$ mechanics-in-time $\to$ field theory), with
probability/transition amplitude language as primary and geometric \(1/2\)-density
...
\item \path{research/workspace/notes/2026-02-09-wikipedia-baseline-definitions-and-skepticism.md} (terminology baseline plus skepticism protocol),
...
\(D=4\)), while keeping track of the oscillatory amplitude expression and its interpretation in the literature and in geometric
...
This is \emph{not} the derivative distribution \(\delta'\) (the distributional derivative of Dirac's \(\delta\)):
it is the pullback/composition of \(\delta\) by the map \(f'\) (or the Euler--Lagrange map).%
\footnote{To avoid a persistent notational trap: the prime in \(\delta(f')\) belongs to \(f'\), not to \(\delta\).
Separately, the symbol ``\(\delta'\)'' appears in point-interaction literature to label contact-interaction families;
that usage is conceptually adjacent (it is also a point-supported sector), but it is not the same as ``differentiate
Dirac's \(\delta\)''.}
The compositional intuition is that amplitudes are the correct objects to compose, not probabilities:
the oscillatory amplitude expression behaves like an amplitude (optionally represented geometrically as a
\(1/2\)-density), and modulus-square produces a density-level
localizing object.
...
\subsection{Canonical Anchors in \texttt{conv\_patched.md}}
...
introduces the oscillatory amplitude (\path{conv\_patched.md:707}), and records the standard nondegenerate identity
\[
...
\paragraph{Near-diagonal scaling and geometric \(1/2\)-densities.}
...
on a Lie groupoid, convolution is naturally formulated using geometric \(1/2\)-densities so that composition is coordinate-free
(\path{conv\_patched.md:967}). This is the conceptual reason the oscillatory amplitude expression is treated as an amplitude-like
object in the program.

\paragraph{Escalation to mechanics and fields.}
For quantum mechanics, the oscillatory object is identified with the propagator amplitude
...
\paragraph{Newton's refinement template (finite invariants before limits).}
The conversation explicitly returns to Newton's area-law proof as the archetype of ``do it discretely, then refine'':
equal areas in equal times are first proved for a polygonal orbit under central impulses, and only then transported to
the smooth limit (\path{conv\_patched.md:2497}). The same passage appears again in action language:
Newton's additivity of swept areas is structurally identical to the additivity of the time-sliced action
(\path{conv\_patched.md:2531}), and ``more triangles, thinner triangles'' is ``more time slices, smaller \(\Delta t\)''
(\path{conv\_patched.md:2537}). This is the conceptual bridge that makes the later ``composition forces exponentials''
argument feel inevitable rather than mystical.

\paragraph{Fixed points as the scale-control locus.}
The transcript already speaks in Wilson--Kogut language: the relevant limit exists only when refinement can be compared
across scales, which is precisely what RG fixed points and their stable/unstable manifolds control
(\path{conv\_patched.md:1164}). It also flags that higher-dimensional generalisation will plausibly exhibit ``more exotic
fixed points'' (\path{conv\_patched.md:816}) and that fixed-point structure is where one expects universal scales to
appear (\path{conv\_patched.md:1426}).
...
\emph{Geometric \(1/2\)-density language is optional at every level:} it is structurally useful for composition statements,
...
\item \textbf{Statics (0D action):}
  prototype object \(\delta(f'(x))\), and stationary-phase control of the oscillatory probability amplitude
  \[
...
\subsection{Foundational Motifs: Refinement, Scaling, and Fixed Points}
\label{sec:motifs}
There are three recurring structural motifs behind almost every successful step in this program.
They are conceptually independent of geometric \(1/2\)-density packaging, but geometric \(1/2\)-densities are often a convenient
coordinate-free way to express them.

\paragraph{(M1) Prove invariants at finite resolution, then take a controlled limit.}
Newton's polygonal proof of the area law is the template:
one proves an \emph{exact} discrete invariant (equal triangle areas per equal time-step) for a finite refinement,
and then one defines the smooth statement by an explicit refinement limit (\(\Delta t\to0\)).%
\footnote{This is exactly the discipline Newton codifies in the ``first and last ratios'' lemmas: the limit is an
explicit definitional step, not an assumption of indivisible geometric atoms.}
For us, the analog is:
define finite-dimensional objects at a regulator (time-slicing, lattice spacing, cutoff \(a\), damping \(\eta\)),
prove exact identities there (projective stability, SD, \(\tau_\mu\) covariance), and then justify the continuum
limit as a theorem.

\paragraph{(M2) Refinement-compatible composition forces exponential weights.}
If an amplitude \(K(t_2,t_0)\) composes under cutting,
\(
K(t_2,t_0)=\int K(t_2,t_1)\,K(t_1,t_0)\,dq(t_1),
\)
then the weight assigned to a history must be multiplicative under concatenation.
Multiplicativity is the reason exponentials appear.
The additive functional singled out by the classical variational principle is the action, hence the canonical weight
\(\exp(\tfrac{i}{\hbar}S)\) at the dynamic level.
At the static level, the same logic is already visible in the Fourier representation of \(\delta(f')\) and its
near-diagonal scaling; and in tangent-groupoid language, it is encoded as ``compose arrows, then rescale the
deformation parameter''.

\paragraph{(M3) When limits fail, scale control moves to RG flows and fixed points.}
In mechanics, time slicing usually converges once the measure is interpreted correctly.
In field theory (especially \(d=4\)), refinement often creates divergences, so one must:
introduce a scale, define renormalized objects relative to that scale, and demand that changing the scale does not
change observables.
In other words, ``existence of the continuum limit'' becomes ``existence of a controlled RG trajectory'';
fixed points and their stable manifolds are where universal behavior and universal scales live.

\paragraph{Point-supported sectors have multiple scaling eigenmodes.}
The delta-of-variation slogan localizes on extrema, but the \emph{local distributional sector} is not one-dimensional.
In 1D, every distribution supported at \(\{0\}\) is a finite linear combination of \(\delta^{(m)}\), and the dilation
pullback satisfies the exact eigenmode law
\[
U_\lambda \delta^{(m)} = \lambda^{-m-1}\,\delta^{(m)}.
\]
Thus the point-supported sector decomposes into multiple scaling channels, each with its own scaling exponent.
This is the minimal mathematical model for ``multiple fixed points'' at the level of local singular data:
depending on regularization and symmetry, channels may be excited, suppressed, or mixed under scaling.

\paragraph{Toy model: 1D contact RG fixed points make the channel picture concrete.}
The local preprint \path{arXiv-hep-th9411081v1/9411081.tex} builds a Wilson--Kogut-style RG directly on the space of
dimensionless cutoff scattering matrices \(\tilde S_{\tilde k}\) in 1D QM.
The RG action is an argument rescaling,
\begin{equation}
\tilde S^t_{\tilde k}=T^t[\tilde S]_{\tilde k}=\tilde S_{e^{-t}\tilde k},
\end{equation}
so fixed points are exactly the constant \(\tilde S\in U(2)\) satisfying the physical conjugation constraint.
Under time-reversal symmetry the fixed points reduce to \(\{\text{a circle}\}\cup\{\pm Q\}\), and the paper interprets
the circle as Kurasov's scale-invariant ``\(\delta'\)'' family.
Linearization around a fixed point yields scaling eigen-directions with
\(
{\vec a}(k)=k^{-n}{\vec a}_0,
\)
so that ``relevant/irrelevant'' is literally the sign of a scaling exponent \(n\) in a point-supported sector.
This is the cleanest toy model we currently have for the Claim~1 intuition:
refinement plus rescaling defines an RG map, and the continuum physics is controlled by fixed points and their stable
manifolds; multiple fixed points are not an accident but a structural feature of point-supported degrees of freedom.

\paragraph{Bridge back to Claim~1.}
Our internal \(\tau_\mu\) flow is the analog of an RG reparameterization: it changes presentation
\((\kappa,\eta,h)\mapsto(\mu\kappa,\eta/\mu,\mu h)\) while keeping the combined parameter \(c\) fixed.
The slogan ``\(c\)-invariance'' is therefore not a rhetorical label: it is the concrete mechanism by which we compare
objects defined at different refinement scales.
The multi-channel fixed-point picture above is a warning and a guide:
Claim~1 can only be true as stated if we can track which scaling channels are present in the local sector and show
that the refinement plus counterterm mechanism drives them into a controlled fixed structure (or at least a controlled
stable manifold) across the three levels.
...
\[
\text{oscillatory amplitude / geometric }\tfrac{1}{2}\text{-density}
\quad\leadsto\quad
...
The oscillatory amplitude object is important because it is the compositional object: it behaves like an amplitude or
geometric \(1/2\)-density under convolution/composition (this is the groupoid-kernel intuition), while the squaring step produces
...
\item selected higher-$D$ GR (Claim~8) and gauge taxonomy (Claim~9) branches have theorem-grade closure in scoped sectors; explicit open sectors are tracked in the audit and claim manuscripts,
...
geometric \(1/2\)-density and groupoid convolution statements certify \emph{kinematic} consistency of composition laws at fixed
...
Terminology skepticism protocol for future agents is tracked in
\path{research/workspace/notes/2026-02-09-wikipedia-baseline-definitions-and-skepticism.md}.
...
The audit also assigns a lightweight maturity score (0--10) to each claim for prioritization.
Table~\ref{tab:claim-maturity} records the current snapshot (as of 2026-02-10 US), extracted from
\path{research/workspace/notes/audits/2026-02-08-top10-claim-audit.md}.
\begin{table}[h]
\centering
\small
...
\hline
\end{tabular}
\caption{Claim maturity scores (0--10) used for prioritization, as recorded in the canonical audit sheet.}
\label{tab:claim-maturity}
\end{table}

As of the latest update, the finite-model Lean hygiene steps are already landed
(including automatic regularity and renormalized-scale wrapper support in the increment chain).
The current queued formal target is to package those templates for direct use in the \(d=3\) bridge closure lane
so that field-level obligations remain explicit and cannot be silently hidden inside toy-model assumptions.
...
\item \path{research/workspace/reports/2026-02-09-claim1-paper1-static-amplitude-qm-equivalence.tex}:
  standalone statics manuscript (probability-amplitude to Born-density equivalence in scoped regime).
\item \path{research/workspace/reports/2026-02-09-claim1-paper2-dynamics-path-integral-equivalence-history.tex}:
  standalone dynamics manuscript (scoped path-integral equivalence plus historical thread).
\item \path{research/workspace/reports/2026-02-09-claim1-paper3-field-theory-general-dimension-roadmap.tex}:
  standalone field-theory manuscript track (dimension-gated closure program, current \(d=3\) candidate status).
\item \path{research/workspace/reports/2026-02-09-claim9-gauge-phase-long-range-paper.tex}:
  standalone Claim~9 manuscript track (phase-resolved long-range gauge potentials with screened-Abelian closure and scoped non-Abelian extraction lanes).
...
\item geometric \(1/2\)-density kinematic vs dynamical counterexample diagnostic:
...
and made the closure obligations explicit (B1--B5). The \(d=2\) proof does not even begin to lift until one has
moment/tightness and denominator control uniformly in \((a,L)\), and a quantitative small-\(\kappa\) continuity bound
for local cylinders. In the current scoped branch, these obligations are now discharged in a concrete interacting
lane and lifted through successive observable/test-class and nonlocal extensions.
\begin{itemize}
...
\item B1--B4 are discharged in a compact-spin interacting Euclidean subclass (AN-23), with hard-cutoff lift
  \(R\to\infty\) in a local-renormalized channel (AN-24), and observable/test-class widening to \(C_b\) and \(C_b^1\)
  via explicit tail/insertion control (AN-25/AN-26/AN-26B).
\item the widened local class is transferred to the oscillatory/de-regularized branch (AN-27), then extended to
  disconnected nonlocal cylinders with refinement-Cauchy bookkeeping and explicit denominator rates (AN-28/AN-29),
  finite graph-indexed multiblock families (AN-30), and exhaustion families (AN-31).
\item the exhaustion-family lane is lifted to weighted-local observable/SD-test classes (AN-32) and then to
  graph-decay nonlocal weighted-local channels with explicit denominator-rate bookkeeping (AN-33); AN-34A replaces
  those rate postulates by first-principles shell-tail envelopes. Lean wrappers (AN-33L/AN-34L) package the
  exhaustion/regularization commuting-limit interface.
\item remaining gap: make the exhaustion/regularization envelopes explicit in the field-side statements (so the Lean
  commuting-limit wrapper can be invoked without hidden hypotheses), and tighten the reconstruction-facing interface
  (Gate G3) in the \(d=3\to d=4\) frontier transition.
\end{itemize}
...
\item geometric \(1/2\)-density statements must be split into kinematic vs dynamical (AR note).
\end{itemize}
...
The short-term queue is designed to be incremental and non-drifting: every target should either reduce assumptions in
an already-mechanized lemma chain, or discharge one of the explicit \(d=3\) bridge obligations, or clarify why a
particular \(d=4\) upgrade is blocked.

Immediate targets:
\begin{itemize}
\item \textbf{\(d=3\) field packaging (current priority):} exhibit the concrete exhaustion/regularization envelopes
  (\texttt{t n}, \texttt{e k}) for the weighted-local graph-decay nonlocal channels and wire them into the
  AN-33L-C commuting-limit wrapper, then synchronize Paper~3 and the corresponding field-side theorem notes.
\item \textbf{Lean alignment for the same lane:} keep the finite surrogate wrappers and the field-side envelope
  definitions synchronized so the commuting-limit invocation is assumption-transparent.
\item \textbf{Newton-limit paradox support lane:} upgrade the current Lean-checked 1D Gaussian semigroup + diagonal
  prefactor anchor to an explicit kernel-level \(t^{-d/2}\) normalization theorem in \(d\) dimensions and connect it
  to the Van Vleck/Schur-complement determinant prefactor lane.
\item \textbf{Claim 9 beyond-window gap:} replace scoped non-Abelian transfer assumptions with first-principles
  control and tighten dynamical-matter string-breaking crossover rigor.
\item \textbf{Cross-level hygiene:} keep geometric \(1/2\)-density composition results separate from continuum existence statements,
  and require every claimed ``lift'' (e.g.\ from \(d=2\) to \(d=4\)) to list its missing analytic gates explicitly.
\end{itemize}

\subsection{Longer-Horizon Conjectures and Risks}
The longer-horizon targets are those that would upgrade Claim~1 from a scoped closure to a full field-level closure
in a genuinely local interacting class. These targets are also the main risk points: they are where the existing
tooling (stationary phase intuition, geometric \(1/2\)-density kinematics, SD identities) must be matched with hard analytic
control (tightness, renormalization, and reconstruction).

The core missing analytic ingredients are:
\begin{itemize}
\item tightness/precompactness mechanisms for cylinder marginals in local interacting field models (especially in \(d=4\)),
\item robust non-vanishing control for oscillatory partition normalizations on complex parameter domains in local interacting classes,
\item SD pass-through with gradient terms (control of insertions and boundary terms) beyond the current scoped lanes,
\item \(d=4\) renormalization flow control and nontriviality criteria (and, if pursued, reconstruction to Minkowski),
...
\subsection{Glossary}
This appendix summarizes the minimal vocabulary used throughout the notes and reports.
The canonical Markdown version lives in \path{research/workspace/notes/2026-02-09-foundational-glossary.md}.
\begin{description}
\item[\(c\)-parameter]
The combined complex parameter
\[
c := (\eta - i/h)\kappa
\]
that governs the dressed kernel family.
\item[\(c\)-invariant]
Invariant under changes of \((\kappa,\eta,h)\) that keep \(c\) fixed; equivalently, constant on a \(\tau_\mu\)-orbit.
\item[\(\tau_\mu\) flow]
...
The one-sided limit \(\eta\to0^+\) from damped oscillatory weights to the purely oscillatory branch.
\item[Schwinger--Dyson identity]
Integration-by-parts identity (scoped finite form):
\[
\langle \partial_i\psi\rangle_c = c\,\langle \psi\,\partial_i S\rangle_c.
\]
\item[probability amplitude (here)]
A complex amplitude-level object whose modulus-square yields density-level quantities.
\item[geometric \(1/2\)-density (here)]
Square-root density bundle object used for coordinate-free groupoid convolution kernels.
\end{description}

\subsection{Reproducibility (Commands and Entry Points)}
This repository is designed so that key statements have executable entry points:
Lean modules are built by \path{lake}, and diagnostics are run with \path{python3.12}.
...
\paragraph{Selected diagnostics.}
\begin{verbatim}
...
\subsection{File Map}
This is a curated entry-point index, not an exhaustive list of all notes produced.
\begin{itemize}
...
\item Claim~1 reports: \path{research/workspace/reports/2026-02-08-claim1-variational-delta-note.tex},
  \path{research/workspace/reports/2026-02-09-claim1-scoped-complete-proof.tex},
  \path{research/workspace/reports/2026-02-09-claim1-paper1-static-amplitude-qm-equivalence.tex},
  \path{research/workspace/reports/2026-02-09-claim1-paper2-dynamics-path-integral-equivalence-history.tex},
  \path{research/workspace/reports/2026-02-09-claim1-paper3-field-theory-general-dimension-roadmap.tex}.
