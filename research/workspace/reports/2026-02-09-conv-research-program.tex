...
the ``halved expression'' behaves like an amplitude (or half-density), and modulus-square produces a density-level
localizing object.
...
They are conceptually independent of the ``half-density'' packaging, but half-densities are often a convenient
...
Immediate targets:
\begin{itemize}
\item \textbf{AN-18 (Lean hygiene):} reduce assumptions in the finite exponential-family increment chain by proving
  automatic regularity and non-vanishing conditions in that model. This keeps the Lean spine tight and prevents
  later reuse from silently importing unjustified hypotheses.
...
\item \textbf{Cross-level hygiene:} keep half-density composition results separate from continuum existence statements,
...
tooling (stationary phase intuition, half-density kinematics, SD identities) must be matched with hard analytic
...
An amplitude-level object whose modulus-square yields density-level quantities; natural for coordinate-free groupoid
convolution kernels.
\end{description}
