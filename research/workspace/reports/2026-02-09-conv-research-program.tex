...
\item a renormalized finite-volume moment channel and a scoped continuum-branch candidate are now explicit,
  but full closure still requires concrete discharge of B1--B4 in one interacting \(d=3\) subclass.
\end{itemize}
...
Immediate targets:
\begin{itemize}
\item \textbf{\(d=3\) closure package (current priority):} discharge B1--B4 in one concrete interacting subclass
  (tightness, denominator non-vanishing, SD insertion control, and topology/observable compatibility) to upgrade
  the current scoped candidate to a scoped closure theorem.
\item \textbf{Lean packaging for the same lane:} keep finite-model formal wrappers aligned with the \(d=3\) closure
  constants so that formal support remains assumption-transparent.
\item \textbf{\(d=3\) bridge closure work:} close at least one beyond-ultralocal \(d=3\) theorem fragment by addressing
  B1--B5 systematically (moments, tightness, denominator control, SD insertion control, and \(\kappa\)-continuity).
  The goal is not an all-at-once \(d=3\) continuum construction, but a sequence of honest theorems that reduce the gap.
