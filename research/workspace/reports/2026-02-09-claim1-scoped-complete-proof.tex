...
It further closes:
(vi) genuinely large-$N$ mode-coupled lifts, including an explicit Gaussian-tail rate, a non-factorized quadratic-mixing determinant class, and a non-factorized quartic-tail class under log-derivative summability,
...
and (ix) finite-dimensional Schwinger-Dyson identities and exact $\tau_\mu$-type scale-flow covariance, unified by an invariant kernel parameter.
...
\begin{theorem}[Non-factorized quadratic-mixing large-$N$ extension]
\label{thm:quadratic-mixing}
Let
\[
S_N(u,v)
=
...
\]
where $u\in\R^m$, $v\in\R^{N-m}$, $D_N(u)=\mathrm{diag}(d_{m+1}(u),\dots,d_N(u))$ with
\[
d_j(u)=\lambda_j+2\beta_j(u),\quad
\beta_j(u)=\sum_{i=1}^m a_{ij}u_i^2,\quad
a_{ij}\ge0,\ \lambda_j\ge\lambda_->0.
\]
Assume coercive $P_m$ and
\[
A_j:=\sum_{i=1}^m a_{ij},\qquad
\sum_{j=m+1}^\infty \frac{A_j}{\lambda_j}<\infty.
\]
Let $K=(\kappa_{jk})_{j,k>m}$ be real symmetric, $K_N$ its principal truncation, and with $\Lambda=\mathrm{diag}(\lambda_j)$ define
\[
...
\]
($P_N$: projection onto indices $m+1,\dots,N$).

Then for bounded cylinder $F_m$,
\[
\omega_{\eps,\eta,N}(F_m)=
\frac{\int e^{-cS_N}F_m(u)\,du\,dv}{\int e^{-cS_N}\,du\,dv},
\qquad c=\eta-i/\eps,\ \eta>0,
\]
converges as $N\to\infty$ (if the limiting denominator is nonzero), and there exists $C_{F_m,\eps,\eta}>0$ such that
\[
|\omega_{\eps,\eta,N'}(F_m)-\omega_{\eps,\eta,N}(F_m)|
\le
C_{F_m,\eps,\eta}\left(
...
Integrate in $v$:
\[
\int_{\R^{N-m}}e^{-c\frac12 v^\top(D_N+K_N)v}dv
=
C_N(c)\det(D_N+K_N)^{-1/2},
\]
with $u$-independent $C_N(c)$ that cancels in normalized ratios. So
\[
\omega_{\eps,\eta,N}(F_m)
=
\frac{\int_{\R^m}e^{-cP_m(u)}F_m(u)\Phi_N(u)\,du}
...
By Theorem \ref{thm:largen-coupled}, $\Phi_N^{\mathrm{diag}}$ has Cauchy tail control by
\[
\sigma_N:=\sum_{j=N+1}^\infty A_j/\lambda_j.
\]

Since $D_N(u)\ge \Lambda_N$, we have
\[
...
\frac{K_1}{1-\theta},
\]
hence $|\Delta_N(u)|\le C_\Delta$ uniformly.

Now set
\[
Q(u):=\mathrm{diag}\!\left((\lambda_j/d_j(u))^{1/2}\right),\qquad 0<Q(u)\le I.
\]
Then $M_\infty(u)=Q(u)\widetilde KQ(u)$ and $M_N(u)=P_NM_\infty(u)P_N$, so
\[
\|M_\infty(u)-M_N(u)\|_1
\le
...
thus
\[
|\Delta_\infty(u)-\Delta_N(u)|\le C_\Delta'\tau_N
\]
uniformly in $u$.
...
\Phi_{N'}-\Phi_N
=
\Delta_{N'}(\Phi_{N'}^{\mathrm{diag}}-\Phi_N^{\mathrm{diag}})
+\Phi_N^{\mathrm{diag}}(\Delta_{N'}-\Delta_N),
\]
and
\[
|\Phi_{N'}(u)-\Phi_N(u)|
\le
C_1e^{B\|u\|^2}\big(\sigma_N+\tau_N\big),
\]
for constants $C_1,B$ independent of $N,u$. Multiplying by
$|e^{-cP_m(u)}|=e^{-\eta P_m(u)}$ gives an integrable envelope by quartic coercivity.
Dominated convergence plus the standard ratio-difference estimate yields convergence and the stated mixed tail rate.
\end{proof}
...
\begin{definition}[\(c\)-invariant quantity]
\label{def:c-invariant}
For parameter triples \((\kappa,\eta,h)\), define
\[
c:=(\eta-i/h)\kappa.
\]
A quantity is called \(c\)-invariant if it is unchanged under any parameter change that leaves \(c\) fixed (equivalently, unchanged along \(\tau_\mu\)-orbits).
\end{definition}
...
\begin{proposition}[Kernel-parameter unification of \texorpdfstring{$\tau_\mu$}{tau_mu} and Schwinger-Dyson]
\label{prop:c-unify}
Set
\[
c:=(\eta-i/h)\kappa.
...
Then:
\begin{enumerate}[label=(\roman*),leftmargin=1.6em]
\item \(\tau_\mu\) preserves \(c\),
\item the Schwinger-Dyson identity in Theorem \ref{thm:sd} depends only on \(c\).
\end{enumerate}
Hence two parameter triples on the same \(\tau_\mu\)-orbit define identical Schwinger-Dyson relations for all admissible observables.
\end{proposition}

\begin{proof}
Part (i) is exactly the computation in Theorem \ref{thm:tau}. For part (ii), Theorem \ref{thm:sd} is written purely in terms of \(\mathcal I_c(\cdot)\), hence only through \(c\). Therefore \(\tau_\mu\)-related parameter triples yield the same SD equations.
\end{proof}
...
\item Theorems \ref{thm:largen-coupled}, \ref{thm:quadratic-mixing}, \ref{thm:quartic-tail}, and Corollary \ref{cor:quartic-intrinsic}: large-$N$ coupled extensions (Gaussian-tail rate, non-factorized quadratic-mixing determinant class, non-factorized quartic-tail class, and intrinsic moment-based sufficient conditions).
...
\item Theorems \ref{thm:sd}, \ref{thm:tau}, and Proposition \ref{prop:c-unify}: Schwinger-Dyson identities, exact scale-flow covariance, and their shared invariant kernel parameter.
\end{enumerate}
...
\item large-$N$ mode-coupled convergence with quantitative tail control (Gaussian-tail, non-factorized quadratic-mixing, and non-factorized quartic-tail classes), with intrinsic moment criteria for quartic-tail hypotheses,
\item non-vanishing control for normalization factors,
\item extension to large observable classes,
\item Schwinger-Dyson identities and scale-flow covariance with a shared \(c\)-invariance structure.
\end{enumerate}
