...
closing dynamical-matter string-breaking rigor. We also add a covariance-based
criterion that reduces transfer assumptions to explicit plaquette-class bounds.
\end{abstract}
...
\item covariance-based sufficient criterion for transfer-channel bounds.
\end{enumerate}
...
\begin{proposition}[Covariance criterion for transfer-channel bounds]
Assume \(\partial_\beta\log\langle W(r,T)\rangle_\beta\) is written as a sum of
normalized plaquette covariances and plaquettes are partitioned into area class
\(\mathcal A\), perimeter class \(\mathcal P\), and remainder class \(\mathcal R\).
If
\[
\left|\frac{1}{|\mathcal A|}\sum_{p\in\mathcal A}\Xi_{\beta,p}\right|\le A_*,
\qquad
\left|\frac{1}{|\mathcal P|}\sum_{p\in\mathcal P}\Xi_{\beta,p}\right|\le B_*,
\qquad
\left|\sum_{p\in\mathcal R}\Xi_{\beta,p}\right|\le R_*,
\]
then AD assumptions (TB-DIFF)+(TB-BOUNDS) hold with channel coefficients
\((a_\beta,b_\beta,R_\beta)\) bounded by \((A_*,B_*,R_*)\).
\end{proposition}
...
The transfer assumptions are now linked to an explicit covariance-criterion checklist.
...
\item \texttt{python3.12 research/workspace/simulations/claim9\_nonabelian\_derivative\_covariance\_check.py}.
\end{itemize}
\end{enumerate}
...
and covariance-based sufficient criterion.
...
extended by an explicit \(\beta\)-transfer theorem and covariance criterion.
