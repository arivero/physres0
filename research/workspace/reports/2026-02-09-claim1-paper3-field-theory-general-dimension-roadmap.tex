...
What remains open is widening that channel to broader local classes.
...
\begin{enumerate}
\item extend the AN-24 compact-support local observable/test classes to broader local classes while preserving B1-B4 controls,
\item keep the same renormalized \(d=3\) SD pass-through channel during that class extension.
\end{enumerate}
...
It now includes scoped \(d=3\) closure with hard-cutoff lift in a local-renormalized channel,
but it is not yet a full broad-class unbounded-field closure report.
