...
The immediate next theorem step is to widen that channel from compact-support local
classes to broader local classes; this AN-25 lane is now active.
\end{abstract}
...
hard-cutoff removal is now closed in a local-renormalized channel and class-widening remains open.
...
AN-25 now starts this widening: observable-side \(C_c\to C_b\) local extension
...
\begin{enumerate}
\item complete AN-25 by closing SD test-side \(C_b^1\) extension (tail insertion-control) in the same \(d=3\) channel,
\item then transfer the widened local class to the oscillatory/de-regularized branch.
\end{enumerate}
...
It now includes scoped \(d=3\) closure with hard-cutoff lift and an active AN-25
class-extension lane; full broad-class unbounded-field closure is not yet complete.
